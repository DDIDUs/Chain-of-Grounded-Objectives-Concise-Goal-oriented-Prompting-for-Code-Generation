...
def _execute(
    ...
    **kwargs,
    ...
    if lang == "python":
        ...
        ext = "py"
        if trace:
            command = f"/bin/sh -c \"timeout {timeout}s /bin/sh -c 'python3 -u -m trace --trace /code.{ext} < /stdin.in; echo Exit Code: $?;' || echo 'Timeout Failed'\""
        else:
            command = f"/bin/sh -c \"timeout {timeout}s /bin/sh -c 'python3 -u /code.{ext} < /stdin.in; echo Exit Code: $?;' || echo 'Timeout Failed'\""
    elif lang == "c":
...
def execute():
    try:
        ...
        return {"error": 'Compile Failed'}, 400
    except Exception as e:
        return {"error": "Compile Failed"}, 500
